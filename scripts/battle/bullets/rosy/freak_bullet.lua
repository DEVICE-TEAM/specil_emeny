...
function bullet:init(x, y, img, telegraph)
    ...
    super.init(self, x, y, "bullets/rosy/freak_bullet_1")
    ...
    Game.battle.timer:after(telegraph + basically_randomness, function()
        self.sprite:setSprite("bullets/rosy/freak_bullet")
        self.sprite:play(1/15, false, function()
            self:remove()
        end)
    end)
...
function bullet:update()
    ...
    if self.sprite.frame == 5 then
        self.damage = 55
        self.can_graze = true
    elseif self.sprite.frame == 6 then
        self.damage = 0
        self.can_graze = false
    end
end
