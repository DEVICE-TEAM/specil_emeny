---@class StaticOutlineFX : FXBase
---@overload fun(...) : StaticOutlineFX
local StaticOutlineFX, super = Class(FXBase)

function StaticOutlineFX:init(color, settings, priority)
...
function StaticOutlineFX:setColor(r, g, b, a)
    self.color = {r, g, b, a}
end

function StaticOutlineFX:getColor()
    return self.color[1], self.color[2], self.color[3]
end

function StaticOutlineFX:isActive()
    return super.isActive(self) and self.amount > 0
end

function StaticOutlineFX:draw(texture)
